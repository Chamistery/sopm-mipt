 ...
 *
 * Mentor uses these to grade each team member after a sprint. POST
 * creates a fresh score; PUT updates an existing one (the mentor edits
 * their previous grade before the sprint closes).
 */
...
/**
 * Validates a score value before sending it to the backend. Returns null
 * when the value is acceptable, otherwise a human-readable Russian
 * message suitable for an inline error.
 */
