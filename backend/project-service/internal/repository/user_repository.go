...
func (r *UserRepository) GetByID(ctx context.Context, id int) (*models.User, error) {
	query := `
		SELECT id, first_name, last_name, COALESCE(middle_name, ''), email, role,
		       COALESCE(company, ''), COALESCE(course, ''), COALESCE("group", ''),
		       COALESCE(avatar, ''), COALESCE(gpa, 0), COALESCE(direction, '')
		FROM users
		WHERE id = $1
	`
